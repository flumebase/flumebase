...
public class TestWindowedHashMap {
  ...
  public void testMap() {
    ...
    refined = map.getRange("foo", Long.valueOf(200), Long.valueOf(300));
    assertEquals(0, refined.size());
    refined = map.getRange("foo", Long.valueOf(500), Long.valueOf(600));
    assertEquals(0, refined.size());
    refined = map.getRange("foo", Long.valueOf(300), Long.valueOf(312));
    assertEquals(0, refined.size());

    // This side of the interval should return a value though.
    refined = map.getRange("foo", Long.valueOf(312), Long.valueOf(315));
    assertEquals(1, refined.size());
    assertEquals(Integer.valueOf(42), refined.get(0));

    // Verify that the correct time range but the wrong key does not include
    // it.
    refined = map.getRange("bar", Long.valueOf(300), Long.valueOf(400));
    assertEquals(0, refined.size());

    // Add another key with a new timestamp.
    map.put("bar", Integer.valueOf(43), Long.valueOf(611));
    // Verify that we can retrieve it.
    refined = map.getRange("bar", Long.valueOf(600), Long.valueOf(700));
    assertEquals(1, refined.size());
    ...
    refined = map.getRange("foo", Long.valueOf(300), Long.valueOf(400));
    assertNotNull(refined);
    assertEquals(1, refined.size());
    assertEquals(Integer.valueOf(42), refined.get(0));
    ...
    refined = map.getRange("baz", Long.valueOf(300), Long.valueOf(400));
    assertNotNull(refined);
    assertEquals(0, refined.size());
    ...
    refined = map.getRange("foo", Long.valueOf(1500), Long.valueOf(3500));
    assertEquals(2, refined.size());
    assertContains(Integer.valueOf(2), refined);
    assertContains(Integer.valueOf(3), refined);
    
    refined = map.getRange("foo", Long.valueOf(500), Long.valueOf(3500));
